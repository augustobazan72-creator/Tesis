...
def guia__archivo_pareo():
    print('='*80)
    print('PAREO PF - SDDP (ARCHIVO.XLSX)')
    print('='*80)
    logger.info('\nEl archivo debe subirse en formato excel.')
    logger.info('El archivo excel debe contar con 3 hojas: "Gen. Syn." - "Gen. Sta." - "Cargas"')
    logger.info('Los nombres de las hojas deben ser los mencionados anteriormente y estar en el mismo orden.')
    logger.info('Cada hoja debe contar con una primera columna "PF" y una segunda "SDDP" donde se relacione')
    logger.info('    el nombre en PowerFactory con el nombre SDDP.')
    logger.info('En el caso de la hoja de generadores sincronos "Gen. Syn." se debe contar con una tercera')
    logger.info('    columna llamada "P_min_MW" en donde se detalle la potencia minima de cada generador.')
    logger.info('Mediante la aopcion 3, puede obtener la informacion de nombres de generadores sincronos,')
    logger.info('    generadores estaticos y cargas.')
    print('-'*80)

def menu_escenarios():
    print(f'{'='*80}')
    print('IMPORTAR ESCENARIOS SDDP A PF.')
    print(f'{'='*80}')
